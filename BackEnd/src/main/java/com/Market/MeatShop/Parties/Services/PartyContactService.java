...
public class PartyContactService {
  ...
  public PartyContact findByIdEn(Long id) {
    return partyContactRepo
        .findById(id)
        .orElseThrow(() -> new TargetNotFound("Party Contact  : " + id + "  not found"));
  }
  ...
  public boolean deleteAllPartyContacts(Long partyId) {
    log.info("attempt to delete all contacts of party {}", partyId);
    partyContactRepo.deleteByPartyId(partyId);
    return true;
  }
}
